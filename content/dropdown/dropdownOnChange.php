...
$data2 = $koneksi->prepare("SELECT paket_id FROM proses_jamaah WHERE id = " . $_GET['id_proses'] . "");
$data2->execute();
$row2 = $data2->fetch();
...
<select onchange="ubahPaket(<?php echo $_GET['id_proses'] ?>, this.value)" class="form-control select2" id="<?php echo explode("_", $_GET['name'])[1]; ?>" name="<?php echo explode("_", $_GET['name'])[1]; ?>">
    ...
    if ($count !== 0) {
        foreach ($row as $dt) {
    ?>
            <option <?php if ($dt['id'] == $row2['paket_id']) echo "selected"; ?> value="<?php echo $dt['id'] ?>"><?php echo $dt['nama'] ?></option>
    <?php
        }
    }
    ?>

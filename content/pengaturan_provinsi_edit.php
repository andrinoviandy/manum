...
if (isset($_POST['id'])) {
    ...
    $sql = "SELECT * FROM provinsi WHERE id = " . $_POST['id'] . "";
    $data = $koneksi->prepare($sql);
    $data->execute();
    $row = $data->fetch();
}
if (isset($_SESSION['id_ubah'])) {
    $sql = "SELECT * FROM provinsi WHERE id = " . $_SESSION['id_ubah'] . "";
    $data = $koneksi->prepare($sql);
    $data->execute();
    $row = $data->fetch();
}
                                ...
                                <div class="card-body row">
                                    <div class="card-body col-lg-12">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" required name="nama_provinsi" value="<?php echo $row['nama_provinsi'] ?>">
                                        </div>
                                    </div>
                                </div>

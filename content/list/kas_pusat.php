...
$sql = "SELECT COUNT(*) OVER () AS total, row_number() OVER (ORDER BY k.id DESC) AS no, SUM(k.nominal) OVER () AS jumlah, k.*, ks.nama_kas FROM keuangan k INNER JOIN kategori_keuangan kk ON kk.id = k.kategori_keuangan_id INNER JOIN kas ks ON ks.id = k.kas_id WHERE kk.id = 4 AND k.proses_jamaah_id = " . $_POST['id_proses'] . " AND ks.cabang_id = " . $_POST['cabang_id'] . " $where ORDER BY k.id DESC LIMIT $limit OFFSET $paging";
...
?>
        <tr>
            <td><?php echo $dt['no'];
                echo "<script>countData1(Math.ceil($dt[total] / $limit), $dt[total])</script>"; ?>
                <!-- <input type="text" id="ceil_data" value="<?php echo ceil($dt['total'] / $limit); ?>"> -->
            </td>
            <td><?php if ($dt['status_konfirmasi'] == 1) {
...
?>
    <tr>
        <td colspan="10" class="text-center">
            ...
            ?>
            <?php echo "<script>countData1(0, 0)</script>"; ?>
        </td>
    </tr>
<?php } ?>
<script>
    ...
    function konfirmasiInvoice(action, objek) {
        Swal.fire({
            title: 'Konfirmasi Invoice !',
            text: action === 1 ? "Kamu Yakin Akan Mengubah Status Menjadi Terkonfirmasi ?" : "Kamu Yakin Akan Membatalkan Status Konfirmasi Invoice Ini ?",
            icon: 'warning',
            showCancelButton: true,
            // confirmButtonColor: '#3085d6',
            // cancelButtonColor: '#d33',
            ...
            if (result.isConfirmed) {
                $.post("manum/action/ubah/konfirmasi.php", objek,
                    function(data, textStatus, jqXHR) {
                        if (data == '1') {
                            ...
                            fetchDataCustom('kas_pusat', {
                                ...
                                cabang_id: objek.next.cabang_id,

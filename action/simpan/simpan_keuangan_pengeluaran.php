...
try {
    ...
    // $row = $dataId->fetch();
    //code...
    $sqlCek = "SELECT * FROM keuangan k INNER JOIN kategori_keuangan kk ON kk.id = k.kategori_keuangan_id WHERE kk.jenis_kategori = 'k' AND k.nomor = '" . $_POST['nomor'] . "'";
    ...
    if ($cekData == 0) {
        ...
        if ($_FILES['bukti']['name'] !== '') {
            $name_file = "pengeluaran_" . date('dmYHis') . "." . explode("/", $_FILES['bukti']['type'])[1];
